...
export const Aboutus = ({ AboutusData }) => {
  return (
    <>
      ...
      <section className="flex flex-col md:flex-row gap-y-10 md:gap-x-10 justify-center items-center px-3 md:px-20 py-10 md:py-0 text-white w-full min-h-screen">

        {AboutusData.map((item, index) => (
          <div
            key={index}
            className="flex flex-1 justify-center md:justify-start items-center md:items-start flex-col gap-3 text-center md:text-left w-full"
          >
            <Heading text={item.AboutusHeading} />
            <SubHeading text={item.AboutusSubheading} />

            <Paragraph
              text={item.AboutusParagraph}
              className={"w-full md:w-[520px] pb-3"}
            />

            <div className="w-full md:w-[520px] flex flex-wrap justify-center md:justify-start gap-6 md:gap-10">
